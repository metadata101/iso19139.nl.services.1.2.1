...
<xsl:stylesheet version="1.0" xmlns:gmd="http://www.isotc211.org/2005/gmd"
    ...
    <xsl:template match="*" mode="metadata">
        ...
        <xsl:for-each select="gmd:identificationInfo/*">
            ...
            <xsl:for-each select="*/gmd:MD_Keywords">
                <xsl:for-each select="gmd:keyword//gmd:LocalisedCharacterString[@locale=$langId]">
                    <Field name="keyword" string="{lower-case(string(.))}" store="true" index="true"/>
                </xsl:for-each>
            ...
            <xsl:for-each select="gmd:pointOfContact/gmd:CI_ResponsibleParty/gmd:organisationName//gmd:LocalisedCharacterString[@locale=$langId]">
                <Field name="orgName" string="{concat(upper-case(substring(string(.),1,1)),substring(string(.), 2))}" store="true" index="true"/>
        ...
        <xsl:for-each select="gmd:distributionInfo/gmd:MD_Distribution">
			...
			<xsl:variable name="protocols" select="'|OGC:CSW|OGC:WMS|OGC:WMTS|OGC:WFS|OGC:WCS|OGC:WCTS|OGC:WPS|OGC:WMC|OGC:GPKG|OGC:OWS-C|OGC:KML|OGC:GML|OGC:WFS-G|OGC:SOS|OGC:SPS|OGC:SAS|OGC:WNS|OGC:ODS|OGC:OGS|OGC:OUS|OGC:OPS|OGC:ORS|INSPIRE Atom|UKST|SPARQL|download|website|'" />
				<xsl:if test="contains($protocols,concat('|',string(string(.)),'|'))">
