  ...
  * render-value taking care of element values (eg. characterString, URL)
    ...
    <dl>
      <dt>
        <xsl:value-of select="if ($fieldName)
                                then $fieldName
                                else tr:node-label(tr:create($schema), name(.), concat($xpath, '/', name(.)))"/>
      </dt>
      <dd>
  ...
  <xsl:template mode="render-field"
    ...
    <xsl:variable name="xpath2">
      <xsl:choose>
        <xsl:when test="ends-with($xpath, name(.))">
  ...
  <xsl:template mode="render-field"
    ...
    <div class="entry name">
      <h3>
        <xsl:value-of select="tr:node-label(tr:create($schema), name(), $xpath)"/>
        ...
        <div class="col-md-6">
          <address>
            <xsl:for-each select="*/gmd:contactInfo/*">
              <xsl:variable name="xpath2">
                <xsl:choose>
                  <xsl:when test="ends-with($xpath, concat(concat(name(../..), '/'), name(..)))">
                    <xsl:value-of select="$xpath"/>
                  </xsl:when>
                  <xsl:otherwise>
                    <xsl:value-of select="concat($xpath, '/', concat(concat(name(../..), '/'), name(..)))"/>
                  </xsl:otherwise>
                </xsl:choose>
              </xsl:variable>
      ...
      <dd>
        ...
        <p>
          <xsl:variable name="xpath3">
            <xsl:choose>
              <xsl:when test="ends-with($xpath2, name(*))">
                <xsl:value-of select="$xpath2"/>
              </xsl:when>
              <xsl:otherwise>
                <xsl:value-of select="concat($xpath2, '/', name(*))"/>
              </xsl:otherwise>
            </xsl:choose>
          </xsl:variable>
          <xsl:apply-templates mode="render-field"
    ...
    <dl class="gn-keyword">
      ...
      <dd>
        <div>
          <ul>
            <!-- <li> -->
              <xsl:apply-templates mode="render-keyword"
                                   select="*/gmd:keyword/*"/>
            <!-- </li> -->
          </ul>
        </div>
  ...
  <!-- ignore the others graphicOverview elements -->
  <xsl:template mode="render-field"
