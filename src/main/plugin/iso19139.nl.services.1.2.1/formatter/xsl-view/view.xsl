...
<xsl:stylesheet version="2.0"
  ...
  <xsl:include href="../../../../../../../xslt/skin/ngr/skin.xsl"/>
  ...
  <xsl:template mode="render-field"
    ...
    <div class="entry name clearfix gn-contact">
      <h3>
...
<xsl:template mode="render-field"
                ...
                priority="105">
  <div itemprop="dataset" itemscope="itemscope" itemtype="http://schema.org/Dataset" class="pull-left">
  ...
  <xsl:variable name="mdTitle2" select="xslUtils:getIndexField(null, $dsUUID, '_defaultTitle','eng')"/>
   ...
   <xsl:if test="string-length($dsUUID)>0">
     ...
     <a href="{$baseurl}/doc/dataset/{$dsUUID}" class="btn btn-sm btn-primary">
       <xsl:choose>
         <xsl:when test="string(../srv:coupledResource[1]/srv:SV_CoupledResource[srv:identifier/gco:CharacterString = $uuidref]/gco:ScopedName)">
           <xsl:value-of select="../srv:coupledResource[1]/srv:SV_CoupledResource[srv:identifier/gco:CharacterString = $uuidref]/gco:ScopedName" />
         </xsl:when>
         <xsl:otherwise>
           <xsl:value-of select="$dsUUID"/>
         </xsl:otherwise>
       </xsl:choose>
     </a><br/>
   </xsl:if>
