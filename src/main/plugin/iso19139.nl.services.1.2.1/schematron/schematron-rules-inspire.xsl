...
<xsl:template match="/">
      <svrl:schematron-output xmlns:geonet="http://www.fao.org/geonetwork"
         ...
         <svrl:active-pattern>
            ...
            <xsl:attribute name="name">Validatie tegen het Nederlands profiel en INSPIRE op ISO 19119 voor services v 1.2.1</xsl:attribute>
            <xsl:apply-templates/>
         </svrl:active-pattern>
...
<xsl:param name="lowercase" select="'abcdefghijklmnopqrstuvwxyz'"/>
   ...
   <!--PATTERN DutchMetadataCoreSetForServicesValidatie tegen het Nederlands profiel en INSPIRE op ISO 19119 voor services v 1.2.1-->
<svrl:text xmlns:geonet="http://www.fao.org/geonetwork"
              xmlns:svrl="http://purl.oclc.org/dsdl/svrl">Validatie tegen het Nederlands profiel en INSPIRE op ISO 19119 voor services v 1.2.1</svrl:text>
   ...
   <xsl:variable name="conformity_Spec_Title_Exsists"
	  ...
	  <!--RULE -->
<xsl:template match="/gmd:MD_Metadata" priority="1005" mode="M8">
...
<xsl:if test="$keyword">
         <svrl:successful-report xmlns:geonet="http://www.fao.org/geonetwork"
		    ...
		    <!--ASSERT -->
<xsl:choose>
         <xsl:when test="$keyword_INSPIRE"/>
         <xsl:otherwise>
            <svrl:failed-assert xmlns:geonet="http://www.fao.org/geonetwork"
                                xmlns:svrl="http://purl.oclc.org/dsdl/svrl"
                                ref="#_{geonet:element/@ref}"
                                test="$keyword_INSPIRE">
               <xsl:attribute name="location">
                  <xsl:apply-templates select="." mode="schematron-select-full-path"/>
               </xsl:attribute>
               <svrl:text>Keyword (ISO nr. 53)  voor INSPIRE servicetype ontbreekt of heeft de verkeerde waarde</svrl:text>
            </svrl:failed-assert>
         </xsl:otherwise>
      </xsl:choose>
...
<xsl:template match="//gmd:MD_Metadata/gmd:dataQualityInfo/gmd:DQ_DataQuality/gmd:report/gmd:DQ_DomainConsistency/gmd:result/gmd:DQ_ConformanceResult/gmd:specification/gmd:CI_Citation"
      ...
      <xsl:variable name="all_conformity_Spec_Titles"
                    select="ancestor::gmd:dataQualityInfo/gmd:DQ_DataQuality/gmd:report/gmd:DQ_DomainConsistency/gmd:result/gmd:DQ_ConformanceResult/gmd:specification/gmd:CI_Citation/gmd:title"/>
...
<xsl:choose>
         ...
         <xsl:otherwise>
		    ...
		    <!--ASSERT -->
<xsl:choose>
         <xsl:when test="$all_conformity_Spec_Titles[normalize-space(*/text()) =  'VERORDENING (EG) Nr. 976/2009 VAN DE COMMISSIE van 19 oktober 2009 tot uitvoering van Richtlijn 2007/2/EG van het Europees Parlement en de Raad wat betreft de netwerkdiensten']"/>
         <xsl:otherwise>
            <svrl:failed-assert xmlns:geonet="http://www.fao.org/geonetwork"
                                xmlns:svrl="http://purl.oclc.org/dsdl/svrl"
                                ref="#_{geonet:element/@ref}"
                                test="$all_conformity_Spec_Titles[normalize-space(*/text()) = 'VERORDENING (EG) Nr. 976/2009 VAN DE COMMISSIE van 19 oktober 2009 tot uitvoering van Richtlijn 2007/2/EG van het Europees Parlement en de Raad wat betreft de netwerkdiensten']">
               <xsl:attribute name="location">
                  <xsl:apply-templates select="." mode="schematron-select-full-path"/>
               </xsl:attribute>
               <svrl:text>Specificatie (ISO nr. 360) ontbreekt of heeft de verkeerde waarde,verwijzen naar de VERORDENING (EG) Nr. 976/2009 VAN DE COMMISSIE van 19 oktober 2009 tot uitvoering van Richtlijn 2007/2/EG van het Europees Parlement en de Raad wat betreft de netwerkdiensten</svrl:text>
            </svrl:failed-assert>
         </xsl:otherwise>
      </xsl:choose>
      <xsl:apply-templates select="*" mode="M8"/>
